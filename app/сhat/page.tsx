...
export default function ChatPage() {
  const meId = "me-dev"; // TODO: заменить на id пользователя из сессии
  ...
  async function openWith(user: SimpleUser) {
    ...
    // перезагрузим список диалогов, чтобы он закрепился слева
    const list = await fetch("/api/chat/threads/list", { cache: "no-store" }).then(r=>r.json());
    setThreads(list);
    const t = (list as ThreadListItem[]).find(x => x.id === threadId) || null;
    setActive(t);

    // загрузим историю и подключим SSE
  ...
  function attachSSE(threadId: string) {
    ...
    es.onerror = () => { /* авто-переподключение браузером */ };
  }
        ...
        <div style={{ display: "grid", gridTemplateColumns: "360px 1fr", minHeight: 560 }}>
          <aside style={{ borderRight: "1px solid #e5e7eb", padding: 12 }}>
            <div style={{ position: "relative" }}>
              ...
              {searching && <div style={{ color:"#6b7280", padding: "4px 0" }}>Поиск…</div>}
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4, marginTop: 6 }}>
              ...
              {!filteredThreads.length && <div style={{ color:"#6b7280", padding: 8 }}>Пока нет диалогов. Найдите сотрудника выше, чтобы начать.</div>}
            </div>
          </aside>
