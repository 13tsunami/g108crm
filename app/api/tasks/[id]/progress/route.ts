...
// ───────────────────────────────────────────────────────────────────────────────
// GET /api/users/:id — отдать пользователя (удобно для формы редактирования)
// ───────────────────────────────────────────────────────────────────────────────
...
/**
 * PATCH /api/users/:id
 * Тело: любой поднабор полей пользователя.
 * Пример:
 * {
 *   "name":"Иван Иванов",
...
export async function PATCH(req: NextRequest, { params }: { params: { id?: string } }) {
  ...
  const boolish = (v: unknown) =>
    typeof v === "boolean" ? v : typeof v === "string" ? ["1", "true", "yes", "on"].includes(v.toLowerCase()) : undefined;
  ...
  // Разрешённые поля (остальные игнорим)
  const data: any = {};
  ...
  if ("role" in body) data.role = optStr(body.role); // роли проверим позже, если нужно
  ...
  try {
    ...
    // Несуществующий пользователь — вдруг Concurrent delete
    if (e?.code === "P2025") {
...
// (опционально) preflight, чтобы не было мусора в консоли
