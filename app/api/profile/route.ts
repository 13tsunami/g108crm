...
export async function GET() {
  ...
  // ⬇️ временный каст: у старого клиента нет этих полей в типах
  const u = me as any;
...
export async function PATCH(req: Request) {
  ...
  if ("methodicalGroups" in body || "groups" in body) {
    data.methodicalGroups = strArr(body.methodicalGroups ?? body.groups);
  }

  // защищённые поля игнорируем
  delete body.name; delete body.email; delete body.role; delete body.username;

  try {
    // ⬇️ временный каст для data
